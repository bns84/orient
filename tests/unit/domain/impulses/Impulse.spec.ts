...
describe('Impulse Core', () => {
  ...
  describe('Impulse Domain Object', () => {
    it('represents raw thought without evaluation', () => {
      const impulse: Impulse = {
        id: 'i10',
        content: { text: 'Idee für Silvester-Deko' },
      ...
      expect(impulse.state).toBe(ImpulseState.DUST);
      expect(impulse.content.text).toBe('Idee für Silvester-Deko');
    });

  it('can have voice transcript', () => {
    const impulse: Impulse = {
      id: 'i2',
      content: { transcript: 'Ich brauche Ideen für Silvester' },
      state: ImpulseState.DUST,
      createdAt: new Date(),
      links: { threadIds: [], entityIds: [] },
      meta: { pinned: false },
    ...
    expect(impulse.content.transcript).toBe('Ich brauche Ideen für Silvester');
    expect(impulse.content.text).toBeUndefined();
  });

  it('can be linked to threads and entities', () => {
    const impulse: Impulse = {
      id: 'i3',
      content: { text: 'Test' },
      state: ImpulseState.DUST,
    ...
    expect(impulse.links.threadIds).toHaveLength(2);
    expect(impulse.links.entityIds).toHaveLength(1);
  });

  it('can have importance hint (user input only)', () => {
    const impulse: Impulse = {
      id: 'i4',
      content: { text: 'Wichtige Idee' },
      state: ImpulseState.DUST,
      createdAt: new Date(),
      links: { threadIds: [], entityIds: [] },
      meta: {
        pinned: true,
        importanceHint: 'HIGH',  // Nur Nutzer-Input
      },
    };
  ...
  it('can have context snapshot', () => {
    ...
    expect(impulse.contextSnapshotId).toBe('ctx1');
  });

  it('can be in CLOUD state (light condensation)', () => {
    const impulse: Impulse = {
      id: 'i6',
      content: { text: 'Verdichtete Idee' },
      state: ImpulseState.CLOUD,
      createdAt: new Date(),
      links: { threadIds: [], entityIds: [] },
      meta: { pinned: false },
    };
